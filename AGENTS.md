...
- **单根节点**：每个 .vue SFC 模板只允许一个根元素（`vue/no-multiple-template-root` 在 lint 强制）。多块内容（如 ProTable + 弹窗）用一个语义容器包裹，且容器要接住 flex 高度链并留出查询区与表格的间距（`flex flex-col flex-1 min-h-0 gap-4`），否则纵向撑满会断、两卡会贴死
