...
**定时任务只执行后端预注册处理器**。`sys_scheduled_task` 只保存任务键、Cron、时区和启停状态，不保存脚本或任意命令。新增处理器时在 `ScheduledTaskRegistry` 显式注册，管理页只能从这份白名单选择。

**调度配置以数据库为权威来源**。每个实例启动时加载启用计划，每 15 秒对账一次，使某个实例上的启停或 Cron 修改能够最终同步到其他实例。同一任务处理器可配多个计划，但执行时按任务键共用 Redis 锁，同一时刻只会有一个实例真正执行。
